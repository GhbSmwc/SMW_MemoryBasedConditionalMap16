...
;they must retain their values (at least across levels).
;
...
;HUD stuff:
	...
	!UsingCustomStatusBar = 1
		;^Using Custom status bar patch? 0 = no, 1 = yes (note that sprite OAM-based HUD is not supported)
	!StatusbarFormat = $02
	...
	; and display elements spans many number of tiles to be written down.
			!Settings_MBCM16_KeyCounterPosX = 25
			!Settings_MBCM16_KeyCounterPosY = 4
	;Static tiles:
	;Note: TileNumb are tile numbers to use, TileProp are properties,
	;which are stored bitwise as %YXPCCCTT.
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	if !UsingCustomStatusBar == 0
		!Settings_MBCM16_KeyCounterTileNumbPos = VanillaStatusBarXYToAddress(!Settings_MBCM16_KeyCounterPosX, !Settings_MBCM16_KeyCounterPosY, !RAM_0EF9)
		
	else
		!Settings_MBCM16_KeyCounterTileNumbPos = PatchedStatusBarXYToAddress(!Settings_MBCM16_KeyCounterPosX, !Settings_MBCM16_KeyCounterPosY, !FreeramFromAnotherPatch_StatusBarTileStart, !StatusbarFormat)
		!Settings_MBCM16_KeyCounterTilePropPos = PatchedStatusBarXYToAddress(!Settings_MBCM16_KeyCounterPosX, !Settings_MBCM16_KeyCounterPosY, !FreeramFromAnotherPatch_StatusBarPropStart, !StatusbarFormat)
	endif